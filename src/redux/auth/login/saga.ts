...
function* loginUser({ payload: { user } }: any) {
  try {
    if (process.env.REACT_APP_DEFAULTAUTH === "firebase") {
      ...
      }> = yield call(postJwtLogin, {
        email: user.email,
        password: user.password,
      });
      setLoggeedInUser(response);
      setLoggeedInUserToken(response);
      yield put(
        authLoginApiResponseSuccess(AuthLoginActionTypes.LOGIN_USER, response)
      );
    ...
    let text = "Sorry, Credentials are not valid";
    yield put(authLoginApiResponseError(AuthLoginActionTypes.LOGIN_USER, text));
  }
}
